...
// --- TABLA 2: CALIFICACIONES (16 COLUMNAS) ---
    ...
    private Table crearTablaCalificaciones() {
        ...
        table.addCell(crearCeldaSpan("PROMEDIO FINAL", 1, 14, TextAlignment.LEFT, true));
        table.addCell(crearCeldaSpan("9.3", 1, 2, TextAlignment.CENTER, true)); 
...
// --- TABLA 3: INASISTENCIAS Y FIRMAS (11 COLUMNAS) ---
    ...
    private Table crearTablaInasistencias() {
        ...
        // Fila de Encabezados de Meses (Rowspan 1, Colspan 1)
        ...
        // Fila de Datos de Inasistencias (Rowspan 1, Colspan 1)
        ...
        // La celda de título abarca 6 filas (Rowspan 6, Colspan 1)
        table.addCell(crearCeldaSpan("FIRMA DEL PADRE O TUTOR", 6, 1, TextAlignment.CENTER, true)); 

        // Firmas (Cada mes ocupa 10 columnas)
        // ⬇️ CORRECCIÓN AQUÍ: Usamos crearCeldaSpan(contenido, 1, 10, ...) 

        // 1. AGOSTO DIAGNÓSTICO (Rowspan 1, Colspan 10)
        table.addCell(crearCeldaSpan("AGOSTO DIAGNÓSTICO", 1, 10, TextAlignment.CENTER, true));

        // Celda de Firma Vacía
        table.addCell(crearCeldaSpan("", 1, 10, TextAlignment.CENTER, false).setHeight(15f));

        // 2. SEPTIEMBRE
        table.addCell(crearCeldaSpan("SEPTIEMBRE", 1, 10, TextAlignment.CENTER, true));

        // Celda de Firma Vacía
        table.addCell(crearCeldaSpan("", 1, 10, TextAlignment.CENTER, false).setHeight(15f));

        // 3. OCTUBRE
        table.addCell(crearCeldaSpan("OCTUBRE", 1, 10, TextAlignment.CENTER, true));
